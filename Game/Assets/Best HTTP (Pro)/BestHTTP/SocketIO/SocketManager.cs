        ...
        void IManager.OnTransportError(ITransport trans, string err)
        {
                ...
                trans.State == TransportStates.Opening)
            {
                    ...
                    Poller != null)
                {
                    ...
                    if (Poller.State == TransportStates.Open)
                    {
                        HTTPManager.Logger.Warning("SocketManager", "Open (Websocket transport failed to upgrade, fallback to polling)");

                        if ((this as IManager).OnTransportConnected(Poller))
                            (this as IManager).OnPacket(new Packet("40"));
                    }
                    // else: poller is opening, it will be open or closed when the response is received
                }
