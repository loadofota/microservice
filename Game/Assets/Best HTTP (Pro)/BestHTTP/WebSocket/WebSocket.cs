            ...
            switch (req.State)
            {
                ...
                case HTTPRequestStates.Finished:
                    if (resp.IsSuccess || resp.StatusCode == 101)
                    {
                        ...
                        HTTPManager.Logger.Warning("WebSocket", string.Format("Request Finished Successfully, but the server sent an error. Status Code: {0}-{1} Message: {2}",
                                                        resp.StatusCode,
                                                        resp.Message,
                                                        resp.DataAsText));
                    break;
                ...
                case HTTPRequestStates.Error:
                    HTTPManager.Logger.Exception("WebSocket", "Request Finished with Error! ", req.Exception);
                    break;

                // The request aborted, initiated by the user.
                case HTTPRequestStates.Aborted:
                    HTTPManager.Logger.Warning("WebSocket", "Request Aborted!");
                    break;

                // Ceonnecting to the server is timed out.
                case HTTPRequestStates.ConnectionTimedOut:
                    HTTPManager.Logger.Error("WebSocket", "Connection Timed Out!");
                    break;

                // The request didn't finished in the given time.
                case HTTPRequestStates.TimedOut:
                    HTTPManager.Logger.Error("WebSocket", "Processing the request Timed Out!");
                    break;
